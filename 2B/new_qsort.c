...
int main(int argc, char **argv) {
    ...
    for (int step = 1; step < size; step = 2 * step) {
        ...
        if (rank + step < size) {            
            ...
            data_t *chunk_received = (data_t *)malloc(total_sizes[rank+step] * sizeof(data_t));
            ...
            MPI_Recv(chunk_received, total_sizes[rank+step], mpi_data_type, rank + step, 1, MPI_COMM_WORLD, &status);
