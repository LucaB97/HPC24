...
int main(int argc, char **argv) {
    ...
    //
    data_t *mydata = (data_t*)calloc(my_total_size, sizeof(data_t));
    if (mydata == NULL) {
    ...
    if (my_total_size > my_own_size) {
        receive_buffer = (data_t*)calloc((my_total_size/2 + 1), sizeof(data_t));
    ...
    if (rank == 0) {
        ...
        //// Memory allocation
        ////
        data_t *data = (data_t*)calloc(N, sizeof(data_t));
        if (data == NULL) {
            fprintf(stderr, "Error: Unable to allocate memory for data on rank %d.\n", rank);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
