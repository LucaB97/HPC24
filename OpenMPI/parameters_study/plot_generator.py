...
def main(test_file):
    ...
    for iterations in iterations_values:
        fig, axs = plt.subplots(3, 1, figsize=(9, 12))  # Two subplots (2 rows, 1 column)
        ...
        for i, warmup in enumerate(warmup_values):
            ...
            axs[1].plot(subset_medium['Size'], subset_medium['Avg Latency(us)'], label=f'Avg Latency (Warmup {warmup})', color=color, marker='o', linewidth=2)
            ...
            axs[2].plot(subset_large['Size'], subset_large['Avg Latency(us)'], label=f'Avg Latency (Warmup {warmup})', color=color, marker='o', linewidth=2)
        ...
        axs[0].set_title(rf'$\mathrm{{Operation:}}$ {operation}, $\mathrm{{Algorithm:}}$ {algorithm}, $\mathrm{{Mapping:}}$ {mapping}' + '\n' + rf'$\mathrm{{Iterations:}}$ {iterations}' + '\n\n' + rf'$\mathrm{{Size < }}$ {threshold_1}$B$')
    ...
    for warmup in warmup_values:
        ...
        for i, iterations in enumerate(iterations_values):
            ...
            axs[1].plot(subset_medium['Size'], subset_medium['Avg Latency(us)'], label=f'Avg Latency (Iterations {iterations})', color=color, marker='o', linewidth=2)
            axs[1].fill_between(subset_medium['Size'], subset_medium['Min Latency(us)'], subset_medium['Max Latency(us)'], color=color, alpha=0.2)
